...
from modules.dataset import ProjectFromPointsOnSphere
__all__ = ['PSConv']
...
class PSConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, resolution, with_se=False, normalize=True, eps=0):
        ...
        self.bandwidths = [64, 16, 64]
        self.transform = ProjectFromPointsOnSphere(self.bandwidths[0])
    ...
    def forward(self, inputs):
        #projectfrompointsonsphere
        x, m, n = self.transform(inputs)
        ...
        #deinterpolate
        fused_features = x + self.point_features(inputs)
