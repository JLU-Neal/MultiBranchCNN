...
def normalize(points):
    for index in range(len(points)):
        max_norm = 0
        for jindex in range(points[index].shape[0]):
...
class ModelNet40Dataset(PointCloudDataset):
    ...
    def __init__(self, config, train=True, orient_correction=True, tranform=None):
        """
    ...
    def load_subsampled_clouds(self, orient_correction):
        ...
        filename = join(self.path, '{:s}_{:.3f}_record.pk'.format(split, self.config.first_subsampling_dl))
        ...
        # Else compute them from original points
            ...
            if self.train:
                names = np.loadtxt(join(self.path, 'modelnet10_train.txt'), dtype=np.str)
            else:
                names = np.loadtxt(join(self.path, 'modelnet10_test.txt'), dtype=np.str)
