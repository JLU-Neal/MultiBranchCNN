...
from datasets.data_utils import load_cls_train_val
...
def normalize(points):
    for index in range(points.shape[0]):
        max_norm = 0
        for jindex in range(points.shape[1]):
            norm = np.sqrt(points[index, jindex, 0]**2 + points[index, jindex, 1]**2 + points[index, jindex, 2]**2)
            if norm > max_norm:
                max_norm = norm
        points[index] /= max_norm
        print(index)
    return points

class ScanNetDataset(PointCloudDataset):

    def __init__(self, config, train=True, orient_correction=True, spherical_transform=None, transforms=None,
        ...
        if self.self_supervision:
            self.points, self.labels = load_cls_train_val(train_files, train=True)
            self.points = self.points[:, :, :3]
        elif train:
            self.points, self.labels = load_cls_train_val(train_files, train=True)
            self.points = self.points[:, :, :3]
        else:
            self.points, self.labels = load_cls_train_val(val_files, train=True)
            self.points = self.points[:, :, :3]
    ...
    def __getitem__(self, idx_list):
        ...
        if self.transform:
            # since the batch is not fixed, we should mannully extract each element from a batch
            transformed_data = self.transform(input_list[0], input_list[15])
            input_list.append(transformed_data)
        return input_list
    ...
    def load_subsampled_clouds(self, orient_correction):
        ...
        # Else compute them from original points
            ...
            for index in range(self.points.shape[0]):
                print('', end='\r')
                print(fmt_str.format('#' * ((index * progress_n) // N), 100 * index / N), end='', flush=True)
                input_points += [self.points[index]]
                input_normals += [self.points[index]]
            print('', end='\r')
...
class ScanNetSampler(Sampler):
    ...
    def __init__(self, dataset: ScanNetDataset, use_potential=True, balance_labels=False):
        ...
        if redo:
            ...
            for epoch in range(10):
                ...
                if breaking:
                    break
            t_finish = time.time()
            print("Neighbors computing time: {:5f}".format(t_finish - t_start))
...
class ScanNetCustomBatch:
    ...
    def __init__(self, input_list):
        # self.transformed_data = torch.from_numpy(input_list[0][-1])
        im_list = input_list[0][-1]
        self.transformed_data = [torch.from_numpy(nparray) for nparray in im_list]
        self.transformed_data = [array.unsqueeze(0) for array in self.transformed_data]
        self.transformed_data = torch.cat(self.transformed_data, dim=0)
    ...
    def pin_memory(self):
        ...
        self.transformed_data = self.transformed_data.pin_memory()
        return self
    ...
    def to(self, device):
        ...
        self.transformed_data = self.transformed_data.to(device)
        return self
