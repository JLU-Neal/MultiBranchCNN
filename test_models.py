...
if __name__ == '__main__':
    ...
    #chosen_log = 'results/Log_2020-10-09_02-07-48'  # => ModelNet40
    ...
    if config.dataset == 'ModelNet40':
        test_dataset = ModelNet40Dataset(config, train=False, tranform=transform)
        test_sampler = ModelNet40Sampler(test_dataset)
        collate_fn = ModelNet40Collate
    elif config.dataset == 'S3DIS':
        test_dataset = S3DISDataset(config, set='validation', use_potentials=True, tranform=transform)
        test_sampler = S3DISSampler(test_dataset)
        collate_fn = S3DISCollate
    elif config.dataset == 'SemanticKitti':
        test_dataset = SemanticKittiDataset(config, set=set, balance_classes=False, tranform=transform)
