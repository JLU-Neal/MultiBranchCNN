...
class ModelTrainer:
    ...
    def __init__(self, net, config, chkp_path=None, finetune=False, on_gpu=True):
        ...
        self.best_mIoU = 0
        self.best_val_ACC = 0
        self.best_vote_ACC = 0
        self.best_sub_mIoU = 0
    ...
    def train(self, net, training_loader, val_loader, config):
        ...
        for epoch in range(config.max_epoch):
            ...
            for batch in training_loader:
                if batch.labels.shape[0] == 1:
                    warnings.warn("The batch size of this step is 1")
                    continue
                ...
                loss = net.module.loss(outputs, batch.labels)
                acc = net.module.accuracy(outputs, batch.labels)
            ...
            # Validation
            net.eval()
            self.validation(net, val_loader, config, save_dict)
            net.train()
    ...
    def validation(self, net, val_loader, config: Config, save_dict=None):

        if config.dataset_task == 'classification':
            self.object_classification_validation(net, val_loader, config, save_dict)
    ...
    def object_classification_validation(self, net, val_loader, config, save_dict):
        """
        ...
        if val_ACC > self.best_val_ACC :
            ...
            self.best_vote_ACC = vote_ACC
            # Checkpoints directory
            checkpoint_directory = join(config.saving_path, 'checkpoints')
            checkpoint_path = join(checkpoint_directory, 'best_chkp.tar')
            torch.save(save_dict, checkpoint_path)
        print('Accuracies : val = {:.1f}% / vote = {:.1f}% best_val = {:.1f}% / best_vote = {:.1f}%'.format(val_ACC,
                                                                                                            vote_ACC,
                                                                                                            self.best_val_ACC,
                                                                                                            self.best_vote_ACC))
        ...
        with open(join(config.saving_path, 'validation.txt'), "a") as file:
            message = 'Accuracies : val = {:.1f}% / vote = {:.1f}% best_val = {:.1f}% / best_vote = {:.1f}%\n'
            file.write(message.format(val_ACC,
                                    vote_ACC,
                                    self.best_val_ACC,
                                    self.best_vote_ACC))
        return C1
    ...
    def cloud_segmentation_validation(self, net, val_loader, config, debug=False):
        ...
        if mIoU > self.best_mIoU:
            self.best_mIoU = mIoU
        print('{:s} mean IoU = {:.1f}% Best IoU = {:.1f}%'.format(config.dataset, mIoU, self.best_mIoU))
    ...
    def slam_segmentation_validation(self, net, val_loader, config, debug=True):
        ...
        if mIoU > self.best_sub_mIoU:
            self.best_sub_mIoU = mIoU
        print('{:s} : best subpart mIoU = {:.1f} %'.format(config.dataset, self.best_sub_mIoU))

        mIoU = 100 * np.mean(val_IoUs)
        print('{:s} :     val mIoU = {:.1f} %'.format(config.dataset, mIoU))
        if mIoU > self. best_val_mIoU:
            self.best_val_mIoU=mIoU
        print('{:s} : best val mIoU = {:.1f} %'.format(config.dataset, self.best_val_mIoU))
