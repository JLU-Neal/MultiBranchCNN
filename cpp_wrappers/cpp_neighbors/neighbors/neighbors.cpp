                                ...
                                float radius)
{
	...
	float d2;
	vector<vector<pair<size_t, float>>> all_inds_dists(queries.size());
	for (size_t i = 0; i < all_inds_dists.size(); i++)
	{
		all_inds_dists[i].reserve(100);
	}
	...
	vector<vector<vector<vector<int>>>> index_array(resolution, vector<vector<vector<int>>>(resolution, vector<vector<int>>(resolution)));
	...
	for (vector<PointXYZ>::iterator it = current_cloud.pts.begin(); it != current_cloud.pts.end(); it++)
	{
		...
		*/
		if (index_array[i][j][k].size()==0)
		{
			index_array[i][j][k].reserve(50);
		}
	...
	for (auto& p0 : queries)
	{
	    ...
	    if (i0 == sum_qb + q_batches[b])
	    {
			...
			//Initialize the array for indexing
			vector<vector<vector<vector<int>>>> temp(resolution, vector<vector<vector<int>>>(resolution, vector<vector<int>>(resolution)));
	...
	//delete index;
	vector<vector<vector<vector<int>>>>().swap(index_array);
	vector<vector<pair<size_t, float>>>().swap(all_inds_dists);
	return;
}
...
int find_neighbors(const float*query_pt, int & index_i, int & index_j, int & index_k,
	...
	vector<vector<vector<vector<int>>>>& index_array,
	vector<PointXYZ> &pts)
{
	int count = 0;
	...
	for (int offset_i=0; offset_i < 3; offset_i++)
	{
		for (int offset_j = 0; offset_j < 3; offset_j++)
		{
			for (int offset_k = 0; offset_k < 3; offset_k++)
			{
				...
				vector<int> &cur_set = index_array[cur_index_i][cur_index_j][cur_index_k];
				//cout <<"resolution: "<<resolution<< "cur_index_i:" << cur_index_i << " cur_index_j" <<cur_index_j<< " cur_index_k:" << cur_index_k << endl;
				for (vector<int>::iterator it = cur_set.begin(); it != cur_set.end(); it++)
				{
